                          ...
                          $rows, $cols, $variables)
    {
        ...
        if (!empty($request['filter'])) {
            ...
            if (count($whereFilter) > 0) {
                $whereF = [];
                foreach($whereFilter as $name => $filters) {
                    $whereF[]='(' . implode(' or ', $filters['query']) . ')';
        ...
        foreach($series as $rowId => $serie0) {
            foreach ($serie0 as $name => $serie) {
                $series1[$name][$rowId] = $serie;
            }
        }
        return response()->json(['sql' => $sql, 'x' => array_values($x), 'series' => $series1, 'whereFilter'=>$whereFilter]);
    }
